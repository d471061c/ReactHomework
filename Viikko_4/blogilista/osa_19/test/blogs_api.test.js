...
describe('Updating a blog', async () => {
  ...
  test('updates a blog', async () => {
    ...
    blogObject.likes = 9999
    await api
      .put(`/api/blogs/${blogObject.id}`)
      .send(blogObject)
      .expect(201)

    const updatedBlog = await Blog.findById(blogObject.id)
    expect(updatedBlog.likes).toBe(blogObject.likes)
  })
})
